...
__global__ void reduce0(int *g_idata, int size)
{
...
int main(int argc, char **argv){
  ...
  switch(atoi(argv[2])){
	...
	std::cout << "reduccion global: " << data_v_i[0] << " en " << reloj.tac() << " ms (da bien?)" << std::endl;
	break;
	...
	std::cout << "reduccion en bloques: " << data_v_i[0] << " en " << reloj.tac() << " ms" << std::endl;
	break;
	...
	sum = thrust::reduce(thrust::device,input,input+size);
  	cout << "reduccion con thrust en device: " << sum 
	     << " en " << reloj.tac() << " ms" << std::endl;
	break;

	case 3:
	relojcpu.tic();
	sum = thrust::reduce(data_h_i.begin(),data_h_i.end());
  	cout << "reduccion con thrust en host: " << sum << " en " << relojcpu.tac() << " ms" << std::endl;
	break;
  }
