...
int main(int argc, char *argv[]) 
{
	...
	switch(caso)
	{
		...
		case 5:
			...
			crono.tic();
	        conv_one_thread_per_output_element_input_in_shared_filter_in_constant<<<N/BLOCK_SIZE,BLOCK_SIZE>>>(d_input, d_output,d_filter);
			break;

		case 6:
			printf("filtro en registro... input en memoria global \n");
			crono.tic();
			conv_one_thread_per_output_element_input_global_filter_in_register<<<N/BLOCK_SIZE,BLOCK_SIZE>>>(d_input, d_output, d_filter);
			break;
	
		default:
		printf("debe seleccionar un caso del 1 al 6\n"); exit(1);
		break;
...
/* compilar: nvcc conv.cu -o conv -lcufft */
