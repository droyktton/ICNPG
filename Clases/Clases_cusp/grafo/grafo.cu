/*
Para el grafo dado en clase CALCULAR:

1 Matriz de adyacencias en formato CSR.
2 Número de caminos de exactamente 5 pasos que unen un vértice con cualquier otro.
3 Número de componentes del grafo, y sus vértices miembros.
4 Colorear, como si fuera un mapa, los vértices usando la mínima cantidad de colores. 
...
typedef cusp::csr_matrix<int,int,cusp::device_memory> csrmatrix;
void power3(csrmatrix &A)
{
  csrmatrix C(A);
  csrmatrix D(A);
  cusp::multiply(A, A, C); // C=A^2
  cusp::multiply(C, A, D); // D=A^3
  cusp::print(D);
}
